...
// MongoDB connection
mongoose.connect(process.env.MONGO_URI || 'mongodb://127.0.0.1:27017/coupons', {
...
app.get('/admin', (req, res) => {
  res.render('admin/dashboard', { user: { name: 'Admin' } });
});
...
app.get('/redeem', async (req, res) => {
  ...
  if (!coupon) return res.render('redeem_result', { 
    title: 'Invalid coupon', 
    message: 'Coupon not found', 
    success: false,
    phone: ''
  });
  if (coupon.isRedeemed) return res.render('redeem_result', { 
    title: 'Already used', 
...
app.post('/redeem/send-otp', async (req, res) => {
  ...
  if (!code) return res.render('redeem_result', { 
    title: 'Error', 
  ...
  const coupon = await Coupon.findOne({ code }).populate('product');
  if (!coupon || coupon.isRedeemed) return res.render('redeem_result', { 
    title: 'Invalid', 
...
app.post('/redeem/verify', async (req, res) => {
  ...
  if (!code || !phone) return res.render('redeem_result', { 
    title: 'Error', 
  ...
  if (!record) return res.render('redeem_result', { 
    title: 'Invalid OTP', 
    message: 'OTP invalid', 
    success: false,
    phone
  });
  if (record.expiresAt < new Date()) return res.render('redeem_result', { 
    title: 'Expired OTP', 
    message: 'OTP expired', 
    success: false,
    phone
  ...
  res.render('redeem_result', { 
    title: 'Success', 
    message: `Coupon redeemed. ${coupon.points} points added to ${phone}.`, 
    success: true,
    phone
  });
});
